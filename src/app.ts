...
import { createLights,createSkyBox } from "./components/env/lighting";
import { loadModels } from "./components/env/class";
import {
  createBoxWithNumber,
  createAnimation,
} from "./components/meshes/hello-mesh";

import { createParticles } from "./components/env/particles";
import { addSounds } from "./components/env/sounds";
...
export class App {
  ...
  /**
   * the authoring data for extension to xr author
   */
  ...
  /**
   * public member which holds the music to be played
   */
  public bgm: Sound;
  ...
  createVideoPlane(scene: Scene, sphere: AbstractMesh) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      const position = videoPlane.position
        .clone()
        .add(posScaled)
        .rotateByQuaternionToRef(rotationQuaternion, new Vector3(0, 0, 0));
  ...
  async createScene() {
    ...
    const O2Model = await SceneLoader.ImportMeshAsync(
      "",
      "assets/models/",
      "O2.glb",
      scene
    ...
    o2Root.position = new Vector3(0, -2.5, 6);

    const HModel = await SceneLoader.ImportMeshAsync(
      "",
      "assets/models/",
      "H2.glb",
      scene
    ...
    hRoot.position = new Vector3(2, -2.5, 6);

    const H2OModel = await SceneLoader.ImportMeshAsync(
      "",
      "assets/models/",
      "H2o.glb",
      scene
    ...
    h2oRoot.addBehavior(dragBehavior);
    ...
    const createDragBehavior = () => {
      ...
      dragBehavior.onDragObservable.add(() => {
        ...
        if (distance <= combineThreshold) {
          ...
          o2Root
            .getScene()
            .beginDirectAnimation(hRoot, [shakeAnimH], 0, 30, false, 1, () => {
    ...
    const boxm10 = createBoxWithNumber(
      scene,
      new Vector3(1, 1, 1),
    ...
    const boxm2 = createBoxWithNumber(
      scene,
      new Vector3(1, 1, 1),
      new Vector3(0, Math.PI / 2, Math.PI / 2)
    );
    ...
    //plus
    const boxm5 = createBoxWithNumber(
      scene,
      new Vector3(1, 1, 1),
      new Vector3(0, Math.PI / 2, Math.PI / 2),
      "+",
    ...
    //equals, mark 8
    const boxm8 = createBoxWithNumber(
      scene,
      new Vector3(1, 1, 1),
      new Vector3(0, Math.PI / 2, Math.PI / 2),
      "=",
    ...
    //label m7
    const vidH2O = await SceneLoader.ImportMeshAsync(
      "",
      "assets/models/",
      "h2o.glb",
      scene
    ...
    //label m1
    const vidcoffee = await SceneLoader.ImportMeshAsync(
      "",
      "assets/models/",
      "coffee.glb",
      scene
    );
    const m1cofani = createAnimation(
      scene,
      vidcoffee.meshes[0],
      this.data,
      "m1"
    ...
    //label m4
    const vidH2 = await SceneLoader.ImportMeshAsync(
      "",
      "assets/models/",
      "H2.glb",
      scene
    ...
    //label m11
    const vidO2 = await SceneLoader.ImportMeshAsync(
      "",
      "assets/models/",
      "O2.glb",
      scene
