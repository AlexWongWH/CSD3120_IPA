...
/**
 * function to create animation in the proved scene and tracker from the video
 * 
 * @param scene The scene where the animation is located
 * @param mesh  the mesh to assign the animation
 * @param app reference to the app class
 * @param id the id of the marker tracker
 * @returns the animation created
 */
...
/**
 * Function to create a cube box mesh with a specfied text on the screen
 * 
 * @param scene the scene where to create the box
 * @param position inital position where to spawn the box
 * @param rotation inital orientation of the box
 * @param text  what text would the box say
 * @param boxColor color of the box
 * @param textColor the color of the text
 * @returns
 */
export function createBoxWithNumber(
  scene: Scene,
  position: Vector3,
  rotation: Vector3,
  text: string = "2",
  boxColor: Color3 = Color3.Green(),
...
export class HelloSphere extends AbstractMesh implements HelloMesh {
  ...
  /**
   * 
   * @param message function to log hello message 
   */
  sayHello(message?: string): void {
  ...
  /**
   * function to register action manager
   */
  private initActions() {
